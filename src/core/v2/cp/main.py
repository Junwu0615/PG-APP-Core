...
class Application(EntryPoint):
    ...
    def _insert_production_order(self, **kwargs) -> int:
        ...
        for _ in range(self.env["NUM_ORDERS"]):
            ...
            _mach_name = random.choice(
                list(
                    k
                    for k, v in self.event_dict["machine_dict"].items()
                    if v["mach_type"] == _prod_type
                )
            )
            ...
            self.ms.add_content(
                topic=f"cp/mach-order/{_mach_name}", payload=payload, qos=1
            )
    ...
    def _init_transaction_dict(self, **kwargs):
        ...
        try:
            ...
            self.logging.error(
                f"[# Rollback] Exception [Code: {e.pgcode}]", exc_info=True
            )
            self.conn.rollback()
