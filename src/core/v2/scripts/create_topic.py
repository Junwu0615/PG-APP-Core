        ...
        try:
            ...
            # TODO 情況 B: Topic 已存在 -> 檢查並對齊狀態
                ...
                if target_p_count > current_p_count:
                    logging.warning(
                        f"管道 {topic_name} 分區不足: {current_p_count} -> {target_p_count}"
                    )
                    ...
                    logging.error(
                        f"拒絕縮減管道 {topic_name} 分區 (Kafka 不支援縮減分區)",
                        exc_info=False,
                    )
                ...
                for key, val in target_configs.items():
                    ...
                    if key not in current_configs or current_configs[key].value != str(
                        val
                    ):
                        updates[key] = str(val)
                ...
                if updates:
                    ...
                    new_resource = ConfigResource(
                        ResourceType.TOPIC, topic_name, set_config=updates
                    )
