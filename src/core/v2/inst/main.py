...
class Application(EntryPoint):
    def __init__(self):
        ...
        _CONSUMER_ORDER_TOPIC = os.getenv(
            "CONSUMER_ORDER_TOPIC", "source.cp.mach-order"
        )
        ...
        _KAFKA_SCHEMA_REGISTRY_HOST = os.getenv(
            "KAFKA_SCHEMA_REGISTRY_HOST", "127.0.0.1:8081"
        )

        _YAML_CONFIGS = parsing_yaml(
            os.path.join("./src/core", f"{_YAML_VERSION}", "factory_config.yaml")
        )
    ...
    def _save_to_sqlite(self, data) -> bool:
        ...
        try:
            ...
            self.logging.notice(
                f'儲存來自 Consumer 訂單訊息 | order id: {data["order_id"]}',
        ...
        try:
            with self.conn:
                ...
                if self.cursor.rowcount > 0:
                    self.logging.info(
                        f"已將 {self.cursor.rowcount} 筆處理中的訂單重置為待處理狀態"
                    )
    ...
    def _get_next_pending_order(self):
        ...
        try:
            with self.conn:
                ...
                if row:
                    self.logging.notice(
                        f"取出排隊順位第一個訂單 | order id: {row[0]}",
                        extra_tags={"order_id": row[0], "step": "1"},
                    )

                    return {"order_id": row[0], "raw_data": json.loads(row[1])}
    ...
    def _mark_order_done(self):
        ...
        try:
            ...
            self.logging.notice(
                f"完成訂單並標記狀態 | order id: {self.order_id}",
    ...
    def _update_order_status(self, **kwargs) -> int:
        ...
        try:
            ...
            if self.event_dict["produced_qty"] >= self.event_dict["target_qty"]:
                ...
                self.kpm.send_message(
                    topic="inst.prod-orders", key=self.mach_name, payload=payload
                )
                ret += 1
                ...
                self.kpm.send_message(
                    topic="inst.status-logs", key=self.mach_name, payload=payload
                )
                ret += 1
                ...
                self.logging.notice(
                    f"[order_id={self.order_id}] have been completed. "
                    f'( produced_qty: {self.event_dict["produced_qty"]} >= target_qty: {self.event_dict["target_qty"]} )'
                )

        finally:
            self.kpm.send_message(
                topic="inst.prod-records", key=self.mach_name, payload=_payload
            )
            ret += 1
            return ret
        ...
        try:
            while not self._stop_event.is_set():
                try:
                    ...
                    if self.event_dict != {}:
                        batch_ct += self._insert_production_record(
                            load_setting["efficiency"]
                        )

                    # 根據 BATCH_SIZE 或 時間間隔 提交事務
                    if (
                        batch_ct >= self.env["BATCH_SIZE"]
            ...
            self.logging.notice(
                f'[{self.env["_MAIN_NAME"]}] '
                f"已強制將緩衝區中所有尚未發送的訊息傳送到 Kafka Broker ...",
                stack_level=0,
            )
    ...
    def run(self):
        ...
        self.start_service(
            self._consumer_message,
            **{
                "title": "消費「主控訂單」訊息服務",
            },
        )
        self.start_service(
            self._producer_message,
            **{
                "title": "生產「邊緣數據」訊息服務",
            },
        )
