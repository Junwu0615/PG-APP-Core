        ...
        if len(event_dict["order_dict"].keys()) > 0:
            ...
            for _order_id in _target_list:
                ...
                if detail["produced_qty"] >= detail["target_qty"]:
                    ...
                    WHERE order_id = %s
                    """,
                        (get_now(hours=8, tzinfo=TZ_UTC_8), _order_id),
                    )
                    ret += 1
                    ...
                    logging.notice(
                        f"[order_id={_order_id}] have been completed. "
...
def insert_production_record(
    cursor, event_dict: dict, _machine_id: int, efficiency: int
) -> int:
    """
    TODO 插入實時生產記錄
        - 狀況 1 : 第一次生產匹配
    ...
    if (
        event_dict["order_queue"][_machine_type]
        and event_dict["machine_status"][_machine_id]["status"] == "IDLE"
    ):
        # 須確認是否已經訂單在身，若無取新訂單
        ...
        ret += 1
        logging.info(
            f"[{_machine_type}: {_machine_id}] Production Begins Based on the Order [{_order_id}]."
        )
...
def init_transaction_dict(conn, cursor) -> dict:
    ...
    try:
        ...
        event_dict["order_queue"] = {
            i: collections.deque() for i in list(set(i[2] for i in products))
        }
    ...
    while True:
        try:
            ...
            for _machine_id in event_dict["machine_status"].keys():
                _ct = insert_production_record(
                    cursor, event_dict, _machine_id, efficiency
                )
            ...
            ret_2 = " | ".join(
                [f"{k}[{len(v)}]" for k, v in event_dict["order_queue"].items()]
            )
